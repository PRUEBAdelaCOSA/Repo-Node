...
* `options` {Object} Required. Supports the following properties:
  ...
  * `reusePort` {boolean} For TCP servers, setting `reusePort` to `true` allows
    bind to a port cross multiple socket if they all set `reusePort`. Incoming
    connections are distributed across the listener sockets. This flag is available
    only on some platforms, such as Linux 3.9+, DragonFlyBSD 3.6+,
    FreeBSD 12.0+, Solaris 11.4, and AIX 7.2.5+. **Default:** `false`.
